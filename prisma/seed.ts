...
async function main() {
  ...
  const sup1 = await prisma.supplier.upsert({
    ...
    update: {},
    create: {
      id: 'sup-1', organizationId: orgId,
      name: 'טכנו-חלקים בע"מ', contactName: 'משה אברמוביץ',
      phone: '03-5551234', email: 'orders@techno-parts.co.il',
  ...
  const sup2 = await prisma.supplier.upsert({
    ...
    update: {},
    create: {
      id: 'sup-2', organizationId: orgId,
      name: 'אוטו-פרו ייבוא', contactName: 'רחל כץ',
      phone: '08-6667890', email: 'rachel@autopro.co.il',
  ...
  const sup3 = await prisma.supplier.upsert({
    ...
    update: {},
    create: {
      id: 'sup-3', organizationId: orgId,
      name: 'אל-קאר שרות', contactName: 'אחמד חסן',
      phone: '04-8889012', email: 'info@alcar.co.il',
  ...
  const sup4 = await prisma.supplier.upsert({
    ...
    update: {},
    create: {
      id: 'sup-4', organizationId: orgId,
      name: 'פרימיום-אוטו', contactName: 'גיל שפירא',
      phone: '02-7771111', email: 'gil@premiumauto.co.il',
  ...
  for (const part of parts) {
    await prisma.part.upsert({
      where: { id: part.id },
      update: {},
      create: { ...part, organizationId: orgId },
    })
  }
  ...
  const orders = [
    ...
    { id: 'wo-6', workOrderNumber: 'WO-2026-0006', status: 'COMPLETED' as const, complaint: 'טיפול שגרתי — 40,000 ק"מ', diagnosis: 'הוחלפו: שמן, פילטרים, בדיקת רצועת טיימינג — תקין', laborHours: 3.5, laborRate: 150, partsTotal: 380, totalPrice: 905, assignedTechnician: 'יוסי כהן', mileage: 38000, customerId: michal.id, vehicleId: mazda.id },
  ...
  for (const wo of orders) {
    await prisma.workOrder.upsert({
      where: { id: wo.id },
      update: {},
      create: { ...wo, organizationId: orgId },
    })
  }
  ...
  const q1 = await prisma.quote.upsert({
    ...
    update: {},
    create: {
      id: 'quote-1', organizationId: orgId, quoteNumber: 'QT-2026-0001', status: 'SENT',
  ...
  const q2 = await prisma.quote.upsert({
    ...
    update: {},
    create: {
      id: 'quote-2', organizationId: orgId, quoteNumber: 'QT-2026-0002', status: 'APPROVED',
  ...
  const q3 = await prisma.quote.upsert({
    ...
    update: {},
    create: {
      id: 'quote-3', organizationId: orgId, quoteNumber: 'QT-2026-0003', status: 'DRAFT',
  ...
  const diagSessions = [
    {
      ...
      aiResponse: JSON.stringify({
        possibleCauses: ['תקלה במסכת אוויר — יתכן דליפת אוויר לאחר מד זרימה אוויר', 'זיהום או תקלה בחיישן מד זרימת האוויר (MAF)', 'פלאג מתדלק פגום או מסתם דלק בעייתי'],
        recommendedTests: ['קריאת קודי שגיאה מלאה עם סורק OBD2', 'בדיקת מד זרימת אוויר עם מולטימטר', 'ריסוס קרבורטור לאיתור דליפות אוויר', 'בדיקת לחץ דלק'],
        commonFixes: ['ניקוי או החלפה של מד זרימת האוויר (MAF)', 'החלפת פילטר אוויר', 'איטום דליפות בצינורות אוויר'],
        estimatedDifficulty: 'MEDIUM', estimatedTime: '2-3 שעות', urgencyLevel: 'MEDIUM',
    ...
    },
    {
      ...
      workOrderId: 'wo-5',
      aiResponse: JSON.stringify({
        possibleCauses: ['מחסור בגז מקרר R-134a עקב דליפה', 'כשל בקומפרסור מיזוג אוויר', 'תקלה ברכיב פנוימטי (שסתום הרחבה)', 'נזק לקונדנסר'],
        recommendedTests: ['בדיקת לחץ גז מקרר עם מד לחצים', 'בדיקת זרם חשמלי לקומפרסור', 'בדיקת UV לאיתור דליפות גז', 'בדיקת מאוורר הקונדנסר'],
        commonFixes: ['מילוי גז מקרר R-134a', 'תיקון דליפות + מילוי גז', 'החלפת קומפרסור', 'החלפת שסתום הרחבה'],
        estimatedDifficulty: 'HIGH', estimatedTime: '3-5 שעות', urgencyLevel: 'HIGH',
  ...
  console.log('✓ Seed complete — org: מוסך הדגמה, user: admin@garage.com / admin123')
  console.log('  6 customers, 10 vehicles, 8 work orders, 4 suppliers, 10 parts, 3 quotes, 4 notes, 2 diagnostics')
}
