...
import { ChevronRight, Phone, Mail, MapPin, Car, Wrench, Plus, Pencil, Rocket, FileText } from 'lucide-react'
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {customer.vehicles.map(v => (
              <div key={v.id} className="relative">
                ...
                />
                <Link
                  href={`/dashboard/customers/${customer.id}/vehicles/${v.id}/documents`}
                  className="absolute top-2 left-2 flex items-center gap-1 text-[10px] text-[#8892a4] hover:text-[#6366f1] transition-colors"
                >
                  <FileText size={11} />מסמכים
                </Link>
              </div>
            ))}
          </div>
