...
import { PeriodicQuoteView }    from '@/components/portal/PeriodicQuoteView'
...
export default async function QuotePage({ params }: { params: { token: string } }) {
  ...
  const periodicData = (q.periodicData as PeriodicPortalData | null) ?? null
  ...
  return (
    <div className="max-w-lg mx-auto px-4 pt-6 space-y-4">
      ...
      {/* Periodic service — tiered interactive view */}
      {periodicData ? (
        <PeriodicQuoteView
          ...
          orgPhone={wo.organization.phone ?? null}
        />
      ) : (
        <>
          {/* Line items — flat quote (non-periodic service) */}
      ...
      {/* Post-approval — guidance on what happens next (flat quotes only; periodic view handles this internally) */}
      {!periodicData && q.status === 'APPROVED' && (
      ...
      {/* Post-rejection — helpful guidance (flat quotes only) */}
      {!periodicData && q.status === 'REJECTED' && (
