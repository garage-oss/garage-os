...
import { Search, User, Car, MessageSquare, PenLine, Wrench, Check, Loader2, ChevronRight, ChevronLeft, X, Phone, Camera, Plus } from 'lucide-react'
...
export function IntakeWizard({ technicians, orgTechRate }: Props) {
  ...
  const [complaint2, setComplaint2] = useState('') // voice appendix
  ...
  async function handleSubmit() {
    setError(null)
    start(async () => {
      // If we have IDs from lookup, use them; otherwise must create customer + vehicle first
      ...
      fd.set('complaint',          [complaint, complaint2].filter(Boolean).join('\n').trim())
        ...
        <div className="space-y-4">
          ...
          <div className="relative">
            ...
            <input
              ...
              autoFocus
            />
            {searching && <Loader2 size={16} className="absolute end-3.5 top-1/2 -translate-y-1/2 animate-spin text-[#8892a4]" />}
          </div>
          ...
          <div className="bg-[#1a1d27] border border-[#2e3147] rounded-xl p-4 space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <input value={customerName} onChange={(e) => setCustomerName(e.target.value)} placeholder="ישראל ישראלי" className="w-full bg-[#252836] border border-[#2e3147] rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-[#6366f1]" readOnly={!!customerId} />
              </div>
              <div>
                <label className="text-xs text-[#8892a4] mb-1 block">טלפון *</label>
                <input value={customerPhone} onChange={(e) => setCustomerPhone(e.target.value)} placeholder="050-0000000" type="tel" className="w-full bg-[#252836] border border-[#2e3147] rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-[#6366f1]" readOnly={!!customerId} />
              </div>
            </div>
          </div>
          ...
          <div className="bg-[#1a1d27] border border-[#2e3147] rounded-xl p-4 space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="col-span-2">
                ...
                <input value={plateInput} onChange={(e) => setPlateInput(e.target.value.toUpperCase())} className="w-full bg-[#252836] border border-[#2e3147] rounded-xl px-3 py-2.5 text-sm font-mono font-bold tracking-widest focus:outline-none focus:border-[#6366f1] uppercase" readOnly={!!vehicleId} />
              </div>
              <div>
                <label className="text-xs text-[#8892a4] mb-1 block">יצרן</label>
                <input value={vehicleMake} onChange={(e) => setVehicleMake(e.target.value)} placeholder="Toyota" className="w-full bg-[#252836] border border-[#2e3147] rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-[#6366f1]" readOnly={!!vehicleId} />
              </div>
              <div>
                <label className="text-xs text-[#8892a4] mb-1 block">דגם</label>
                <input value={vehicleModel} onChange={(e) => setVehicleModel(e.target.value)} placeholder="Corolla" className="w-full bg-[#252836] border border-[#2e3147] rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-[#6366f1]" readOnly={!!vehicleId} />
              </div>
              <div>
                <label className="text-xs text-[#8892a4] mb-1 block">שנה</label>
                <input value={vehicleYear} onChange={(e) => setVehicleYear(e.target.value)} type="number" min="1990" max="2030" className="w-full bg-[#252836] border border-[#2e3147] rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-[#6366f1]" readOnly={!!vehicleId} />
              </div>
              <div>
                <label className="text-xs text-[#8892a4] mb-1 block">קילומטראז׳</label>
                <input value={mileage} onChange={(e) => setMileage(e.target.value)} type="number" placeholder="85000" className="w-full bg-[#252836] border border-[#2e3147] rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-[#6366f1]" />
              </div>
            </div>
